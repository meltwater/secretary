...
func pemWrite(key *[32]byte, path string, pemType string, fileMode os.FileMode) {
	...
	util.Check(os.MkdirAll(filepath.Dir(path), 0775), "Failed to create directory %s", filepath.Dir(path))
	util.Check(ioutil.WriteFile(path, []byte(pemData), fileMode), "Failed to write file %s", path)
}

// Deserialize a PEM file to a NaCL key
func PemRead(path string) *[32]byte {
	encoded, err := ioutil.ReadFile(path)
	util.Check(err, "Failed to read key from %s", path)

	key, err := PemDecode(string(encoded))
	util.Check(err, "Expected key %s to be at least 32 bytes", path)
	return key
}
...
func FindKey(locations ...string) *[32]byte {
	for _, location := range locations {
		...
		if encoded != "" {
			...
			util.Check(err, "Failed to decode key in $%s", location)
			return key
		}
...
func RequireKey(name string, locations ...string) *[32]byte {
	...
	util.AssertThat(key != nil, "Failed to find a %s key", name)
	return key
}
...
func Genkey(publicKeyFile string, privateKeyFile string) {
	...
	util.Check(err, "Failed to generate key pair")
